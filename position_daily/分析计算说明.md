...
> 当前版本包含：**申万二级（RQ）** + **通联 tldb 扩展分析**（估值、资金流、盈利预期、中信行业、主题、宽基风格、单票贡献）
...
| 估值暴露 | 组合偏贵还是偏便宜 | 通联 `mkt_equd_eval` |
| 资金流向 | 超大单/大单是否在加仓 | 通联 `mkt_equ_mf_new` |
| 盈利预期 | 预测市盈率与覆盖度 | 通联 `mkt_equd_eval.PE_CM`（无一致预期滚动表） |
| 中信行业 | 另一套行业权重与超额 | `md_inst_type`（中信）+ `mkt_idxd_citic` |
| 主题/概念 | 战略新兴产业集中度 | `md_inst_type`（战略性新兴产业 2018） |
| 宽基风格 | 300/1000/大小盘暴露 | `rq_base_index` + 通联 `mkt_idxd` |
| 单票贡献 | 今天谁拖后腿 | 持仓快照自身 |
...
| Database | `tldb` | `WIND_MYSQL_DATABASE` |

实现：`wind_db.py`。快照日若无数据，各表自动回退到 `MAX(TRADE_DATE) <= 快照日`。  
表目录见根目录 [`_tlnew_utf8.csv`](../_tlnew_utf8.csv)。

**单位**：`mkt_equd_eval.MARKET_VALUE` 为**元**；报告展示「亿」= `MARKET_VALUE / 1e8`。  
**涨跌**：`mkt_idxd*.CHG_PCT` 为小数，分析中×100 转为百分数。
...
| 模块 | 文件 | 核心公式 |
|------|------|----------|
| 估值 | `wind_analysis.analyze_valuation` | `mkt_equd_eval` 加权 PE_T/PB；市值分位 = 全 A 截面 rank |
| 资金流 | `wind_analysis.analyze_moneyflow` | `mkt_equ_mf_new` NET_FLOW_XL / NET_FLOW_L |
| 盈利预期 | `wind_analysis.analyze_consensus` | PE_CM；无净利润滚动变化 |
| 中信行业 | `wind_analysis.analyze_citics_industry` | `md_inst_type` 中信二级 + `mkt_idxd_citic` |
| 主题 | `wind_analysis.analyze_theme` | 战略性新兴产业(2018) 二级；单票可多主题 |
| 宽基 | `style_analysis.analyze_style` | `rq_base_index` 分桶 + `mkt_idxd` CHG_PCT |
...
| `position_daily/services/config.py` | Mongo / 通联连接 |
| `position_daily/services/wind_db.py` | tldb 查询工具 |
| `position_daily/services/wind_analysis.py` | 通联五类分析 |
