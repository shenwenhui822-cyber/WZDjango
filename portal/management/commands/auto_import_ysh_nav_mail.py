...
「资产净值公告_SAHK33_吾执一三号私募证券投资基金_{YYYY-MM-DD}」
的邮件，从附件 xlsx 解析净值表，写入 fund_nav_real.WZ_YSH_MASTER。
附件可为多行（含 A/B 份额）；仅落库产品代码为 SAHK33 的行。
...
调度：alpha_mail_scheduler 默认 12:10（环境变量 YSH_NAV_MAIL_SCHEDULER_ENABLED）。

用法：
  python manage.py auto_import_ysh_nav_mail
  python manage.py auto_import_ysh_nav_mail --force
  python manage.py auto_import_ysh_nav_mail --nav-date 2026-05-07
"""
...
from portal.services.ysh_nav_mail_service import (
    build_ysh_nav_mail_subject,
    get_ysh_fund_product,
)


def _pick_ysh_nav_xlsx(files: list[Path]) -> Path | None:
    """优先文件名含 SAHK33 / 吾执一三号 / 资产净值公告 的 Excel。"""
    if not files:
        return None
...
class Command(BaseCommand):
    help = (
        "仅运行日为交易日时执行：抓取吾执一三号 SAHK33 资产净值公告邮件 xlsx 并写入 "
        "fund_nav_real.WZ_YSH_MASTER（同表仅导入 SAHK33）；nav_date 默认为运行日之前最近一个交易日。"
    )
    ...
    def _send_result_email(
        ...
        mail_subject = (
            f"[{status}] 吾执一三号净值邮件导入 "
            f"{timezone.localdate().strftime('%Y-%m-%d')}"
        )
        title = "吾执一三号 SAHK33（fund_nav_real / WZ_YSH_MASTER）自动导入结果"
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            target_subject = build_ysh_nav_mail_subject(nav_iso)
            ...
            ymd = nav_iso.replace("-", "")
            save_root = Path(settings.ALPHADATA_DIR) / "ysh_nav_mail" / ymd
            ...
            try:
                ...
                report["message"] = (
                    f"已落库 nav_date={doc['nav_date']}（SAHK33）<- {fp.name}"
                )
