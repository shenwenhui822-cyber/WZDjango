...
写入 fund_nav_real.WZ_BSEE_MASTER（与基金净值页博士一号/泽鑫同结构字段）。
...
class Command(BaseCommand):
    help = (
        ...
        "fund_nav_real.WZ_BSEE_MASTER；nav_date 默认为运行日之前最近一个交易日。"
    )
    ...
    def _send_result_email(
        ...
        body = "\n".join(
            [
                "fareport 吾执二二号净值（fund_nav_real / WZ_BSEE_MASTER）自动导入结果",
                "",
                f"状态: {status}",
