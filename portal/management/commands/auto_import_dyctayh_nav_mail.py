...
调度：alpha_mail_scheduler 默认 14:10（环境变量 DYCTAYH_NAV_MAIL_SCHEDULER_ENABLED）。
...
class Command(BaseCommand):
    ...
    def _send_result_email(
        ...
        body = "\n".join(
            [
                "多元 CTA 一号 SXE021(总)（fund_nav_real / WZ_DYCTAYH_MASTER）自动导入结果",
                "",
                f"状态: {status}",
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            target_subject = build_dyctayh_nav_mail_subject(nav_iso)
            ...
            try:
                ...
                fp = pick_dyctayh_nav_excel(files)
                if not fp:
                    report["status"] = "FAILED"
