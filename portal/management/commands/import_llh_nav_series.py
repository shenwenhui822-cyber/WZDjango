"""
将吾执零零号 SNP584「净值序列」Excel（多行）导入 MongoDB：fund_nav_real.WZ_LLH_MASTER。

表头须含：净值日期或日期、产品代码、产品名称、单位净值、累计单位净值等（同 portal.services.fund_nav_real_service._HEADER_KEYS）。
同表含 A/B 行时仅导入产品代码 SNP584 且产品名称为「吾执零零号私募证券投资基金」的行。
落库与 auto_import_llh_nav_mail 相同（nav_date 唯一 upsert）。
...
from portal.services.llh_nav_mail_service import get_llh_fund_product


class Command(BaseCommand):
    help = (
        "导入吾执零零号 SNP584 净值序列 Excel 到 fund_nav_real，"
        "集合为 settings.NAV_REAL_WZ_LLH_MASTER（默认 WZ_LLH_MASTER）；忽略非主基金行。"
    )
