"""
将吾执量化精选一号 SASQ16「净值序列」Excel（多行）导入 MongoDB：fund_nav_real.WZ_LHJXYH_MASTER。
...
与 auto_import_wz_lyh_nav_mail 中 SASQ16 落库集合一致。

用法：
  python manage.py import_lhjxyh_nav_series --file "xxx.xlsx"
"""
...
class Command(BaseCommand):
    help = (
        "导入吾执量化精选一号 SASQ16 净值序列 Excel 到 fund_nav_real，"
        "集合为 settings.NAV_REAL_WZ_LHJXYH_MASTER（默认 WZ_LHJXYH_MASTER）。"
    )
