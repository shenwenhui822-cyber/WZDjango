...
用法：
  ...
  python manage.py auto_import_slh_nav_mail --force
  python manage.py auto_import_slh_nav_mail --nav-date 2026-05-06
"""
...
from portal.services.slh_nav_mail_service import (
    build_slh_nav_mail_subject,
    get_slh_fund_product,
)
...
def _pick_slh_nav_xlsx(files: list[Path]) -> Path | None:
    """优先文件名含 SXN031 / 吾执三零号 / 【基金净值】的 Excel。"""
    if not files:
        return None
...
class Command(BaseCommand):
    help = (
        "仅运行日为交易日时执行：抓取吾执三零号 SXN031(总) 基金净值邮件 xlsx 并写入 "
        "fund_nav_real.WZ_SLH_MASTER；nav_date 默认为运行日之前最近一个交易日。"
    )
    ...
    def _send_result_email(
        ...
        mail_subject = (
            f"[{status}] 吾执三零号净值邮件导入 "
            f"{timezone.localdate().strftime('%Y-%m-%d')}"
        )
        title = "吾执三零号 SXN031(总)（fund_nav_real / WZ_SLH_MASTER）自动导入结果"
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            target_subject = build_slh_nav_mail_subject(nav_iso)
            ...
            ymd = nav_iso.replace("-", "")
            save_root = Path(settings.ALPHADATA_DIR) / "slh_nav_mail" / ymd
