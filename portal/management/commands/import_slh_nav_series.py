"""
将吾执三零号 SXN031「净值序列」Excel（多行）导入 MongoDB：fund_nav_real.WZ_SLH_MASTER。

表头须含：日期、资产代码、资产名称、资产份额净值、资产份额累计净值等（同 portal.services.fund_nav_real_service._HEADER_KEYS）。
落库与 auto_import_slh_nav_mail 相同（nav_date 唯一 upsert）。

用法：
  python manage.py import_slh_nav_series --file "xxx.xlsx"
"""
...
from portal.services.slh_nav_mail_service import get_slh_fund_product


class Command(BaseCommand):
    help = (
        "导入吾执三零号 SXN031 净值序列 Excel 到 fund_nav_real，"
        "集合为 settings.NAV_REAL_WZ_SLH_MASTER（默认 WZ_SLH_MASTER）。"
    )
