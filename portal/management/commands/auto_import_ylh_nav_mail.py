"""
交易日 14:30 拉取 wangkan（ALPHA_MAIL_*）邮箱中主题
「【基金净值】SQL632(总)_吾执一零号私募证券投资基金_{YYYY-MM-DD}」
的邮件，从附件 xlsx/xls 解析表头行净值表，写入 fund_nav_real.WZ_YLH_MASTER。
...
最近一个交易日（T-1，与 auto_import_dylx_nav_mail 一致）。

调度：alpha_mail_scheduler 默认 14:30（环境变量 YLH_NAV_MAIL_SCHEDULER_ENABLED）。

用法：
  python manage.py auto_import_ylh_nav_mail
  python manage.py auto_import_ylh_nav_mail --force
  python manage.py auto_import_ylh_nav_mail --nav-date 2026-05-07
"""
...
from portal.services.ylh_nav_mail_service import (
    build_ylh_nav_mail_subject,
    get_ylh_fund_product,
)


def _pick_ylh_nav_xlsx(files: list[Path]) -> Path | None:
    """优先文件名含 SQL632 / 吾执一零号 / 【基金净值】的 Excel。"""
    if not files:
        return None
...
class Command(BaseCommand):
    help = (
        "仅运行日为交易日时执行：抓取吾执一零号 SQL632(总) 基金净值邮件并写入 "
        "fund_nav_real.WZ_YLH_MASTER；nav_date 默认为运行日之前最近一个交易日。"
    )
    ...
    def _send_result_email(
        ...
        mail_subject = (
            f"[{status}] 吾执一零号净值邮件导入 "
            f"{timezone.localdate().strftime('%Y-%m-%d')}"
        )
        title = "吾执一零号 SQL632(总)（fund_nav_real / WZ_YLH_MASTER）自动导入结果"
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            target_subject = build_ylh_nav_mail_subject(nav_iso)
            ...
            ymd = nav_iso.replace("-", "")
            save_root = Path(settings.ALPHADATA_DIR) / "ylh_nav_mail" / ymd
