...
DEFAULT_ACCOUNT_ID = "80017209"
SUBJECT_TEMPLATE = "{account_id}华泰期货结算数据{ymd}"
ZIP_NAME_TEMPLATE = "{account_id}-{ymd}.zip"
...
def _save_target_zip_attachment(
    ...
    *,
    ...
    exact_name = ZIP_NAME_TEMPLATE.format(account_id=account_id, ymd=ymd)
    exact_pattern = re.compile(rf"^{re.escape(exact_name)}$", re.IGNORECASE)
    loose_pattern = re.compile(rf"^{re.escape(account_id)}-\d{{8}}\.zip$", re.IGNORECASE)
    ...
    if candidate_payload:
        ...
        return save_path
    raise RuntimeError(f"邮件中未找到目标 ZIP 附件（{exact_name}）。")
...
def _ymd_to_date(ymd: str) -> date:
    return date.fromisoformat(_ymd_to_iso(ymd))
...
class Command(BaseCommand):
    help = "按主题下载华泰期货结算 ZIP，解析资金状况/持仓汇总并写入 future_settle_real.HTQH-80017209"
    ...
    def _send_result_email(self, report: dict[str, object], started_at, ended_at, base_url: str) -> None:
        ...
        subject = f"[{status}] 华泰期货结算单入库 {timezone.localdate().strftime('%Y-%m-%d')}"
        title = "华泰期货结算单自动入库执行结果"
        ...
        body = format_mail_job_notify_body(
            ...
            field_rows=[
                ...
                ("ZIP 文件", report.get("source_zip_file")),
        ...
        snap.update(
            {
                ...
                "source_zip_file": report.get("source_zip_file") or "",
    ...
    def handle(self, *args, **options):
        ...
        target_subject = SUBJECT_TEMPLATE.format(account_id=account_id, ymd=ymd)
        report["target_subject"] = target_subject
        # 检索窗口以目标主题日期为基准，避免补跑历史日期时被“今天-回看天数”错误截断。
        since_date = _ymd_to_date(ymd) - timedelta(days=lookback_days)
        ...
        try:
            try:
                ...
                attach_root = Path(
                    getattr(
                        settings,
                        "HTQH_SETTLE_ATTACH_DIR",
                        settings.BASE_DIR / "downloaded_attachments_htqh",
                    )
                )
                attach_dir = attach_root / ymd
                zip_path = _save_target_zip_attachment(
                    mailbox,
                    mail_id,
                ...
                report["source_zip_file"] = zip_path.name
                self.stdout.write(self.style.SUCCESS(f"已下载目标 ZIP: {zip_path}"))

                parsed = extract_htqh_record_from_zip(
                    zip_path,
                    account_id=account_id,
                    ymd=ymd,
                )
