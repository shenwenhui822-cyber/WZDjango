...
的邮件，从附件 xlsx/xls 解析净值表，写入 fund_nav_real.WZ_LLH_MASTER。
同表多行时仅落库产品代码为 SNP584 且产品名称为「吾执零零号私募证券投资基金」的主基金行（不含 A/B 子份额行）。
...
class Command(BaseCommand):
    help = (
        ...
        "fund_nav_real.WZ_LLH_MASTER（同表仅主基金全称行）；nav_date 默认为运行日之前最近一个交易日。"
    )
    ...
    def _send_result_email(
        ...
        title = "吾执零零号 SNP584（fund_nav_real / WZ_LLH_MASTER）自动导入结果"
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            try:
                ...
                report["message"] = (
                    f"已落库 nav_date={doc['nav_date']}（SNP584 主基金行）<- {fp.name}"
                )
