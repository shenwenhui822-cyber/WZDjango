"""
交易日 11:40 拉取 wangkan（ALPHA_MAIL_*）邮箱中主题
「资产净值公告_SXR194_吾执九零号私募证券投资基金_{YYYY-MM-DD}」
的邮件，从附件 xlsx/xls 解析净值表，写入 fund_nav_real.WZ_JLH_MASTER。
附件可为多行（含 A/B 份额）；仅落库产品代码为 SXR194 的行。
...
调度：alpha_mail_scheduler 默认 11:40（环境变量 JLH_NAV_MAIL_SCHEDULER_ENABLED）。

用法：
  python manage.py auto_import_jlh_nav_mail
  python manage.py auto_import_jlh_nav_mail --force
  python manage.py auto_import_jlh_nav_mail --nav-date 2026-05-07
"""
...
from portal.services.jlh_nav_mail_service import (
    build_jlh_nav_mail_subject,
...
def _pick_jlh_nav_xlsx(files: list[Path]) -> Path | None:
    """优先文件名含 SXR194 / 吾执九零号 / 资产净值公告 的 Excel。"""
    if not files:
        return None
...
class Command(BaseCommand):
    help = (
        "仅运行日为交易日时执行：抓取吾执九零号 SXR194 资产净值公告邮件 xlsx 并写入 "
        "fund_nav_real.WZ_JLH_MASTER（同表仅导入 SXR194）；nav_date 默认为运行日之前最近一个交易日。"
    )
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            target_subject = build_jlh_nav_mail_subject(nav_iso)
            ...
            ymd = nav_iso.replace("-", "")
            save_root = Path(settings.ALPHADATA_DIR) / "jlh_nav_mail" / ymd
            ...
            try:
                ...
                report["message"] = (
                    f"已落库 nav_date={doc['nav_date']}（SXR194）<- {fp.name}"
                )
